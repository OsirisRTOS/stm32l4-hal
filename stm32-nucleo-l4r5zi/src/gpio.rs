...
impl<Mode> GPIOPin<Mode> {
    ...
    pub fn release(self) {
        ...
        let idx = if (self.pin as usize) < 8 { idx } else { idx + 1 };
        let bit = if (self.pin as usize) < 8 { self.pin as usize } else { self.pin as usize - 8 };
        let mask = !(1 << bit);
...
impl<Mode: Configured> GPIOPin<Mode> {
    ...
    pub fn read_state(&self) -> bool {
        ...
        if (idr & target) == 0 { false } else { true }
    }
}
...
impl GPIOPin<Undefined> {
    ...
    pub fn take(port: Port, pin: Pin) -> GPIOPin<Undefined> {
        ...
        let idx = if (pin as usize) < 8 { idx } else { idx + 1 };
        let bit = if (pin as usize) < 8 { pin as usize } else { pin as usize - 8 };
        let mask = 1 << bit;
...
impl<Mode: InputConvertible> IntoInput<GPIOPin<Input>, GPIOPin<Mode>> for GPIOPin<Mode> {
    ...
    fn into_input(self) -> Result<GPIOPin<Input>, GPIOPin<Mode>> {
        ...
        let res = atomic
            .fetch_update(Ordering::SeqCst, Ordering::SeqCst, |value| Some(value & (!target)));
        match res {
            Ok(_) => Ok(GPIOPin::<Input> { pin: self.pin, port: self.port, _mode: PhantomData }),
            Err(_) => Err(self),
        }
    }
}
...
impl<Mode: OutputConvertible> IntoOutput<GPIOPin<Output>, GPIOPin<Mode>> for GPIOPin<Mode> {
    ...
    fn into_output(self) -> Result<GPIOPin<Output>, GPIOPin<Mode>> {
        ...
        match res {
            Ok(_) => Ok(GPIOPin::<Output> { pin: self.pin, port: self.port, _mode: PhantomData }),
            Err(_) => Err(self),
        }
    }
}
...
impl<Mode: AnalogConvertible> IntoAnalog<GPIOPin<Analog>, GPIOPin<Mode>> for GPIOPin<Mode> {
    ...
    fn into_analog(self) -> Result<GPIOPin<Analog>, GPIOPin<Mode>> {
        ...
        match res {
            Ok(_) => Ok(GPIOPin::<Analog> { pin: self.pin, port: self.port, _mode: PhantomData }),
            Err(_) => Err(self),
        }
    }
}

impl<Mode: AlternateConvertible> IntoAlternate<GPIOPin<Alternate>, GPIOPin<Mode>>
    for GPIOPin<Mode>
{
    ...
    fn into_alternate(self) -> Result<GPIOPin<Alternate>, GPIOPin<Mode>> {
        ...
        match res {
            Ok(_) => {
                Ok(GPIOPin::<Alternate> { pin: self.pin, port: self.port, _mode: PhantomData })
            }
            Err(_) => Err(self),
        }
    }
